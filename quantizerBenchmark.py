...
from utils import AverageMeter, lin2img, psnr_ssim, rescale_img, get_val_dataset
...
def reload_trained_model(args):
    ...
    return model, device, model_args, img_resolution
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--evaluation_dataset",
        help="Which dataset must be used to evaluate the model. If None, the test dataset used for training is selected",
        type=str,
        default=None,
    )
    ...
    parser.add_argument(
        "--data_root",
        help="Needed if evaluation_dataset is not None. Path of data",
        type=str,
        default=None,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None, 
        type=int, 
        help='Evaluate only the image at the given index. If None, all images are compressed. Onlu for DIV2k and KODAK')


    args = parser.parse_args()

    
    run_id = random.randint(1, 1000)
    print(f"Run id: {run_id}")
    logdir = os.path.join(args.save_dir, str(run_id))

    model, device, model_args, img_resolution = reload_trained_model(args)

    if args.evaluation_dataset is not None:
        assert args.data_root is not None, "Please provide data root"
        ds = utils.get_val_dataset(args.evaluation_dataset, args.data_root, idx = args.image_index)
        assert ds is not None, "Please provide a valid dataset (CIFAR10, DIV2K, KODAK)"
    else:
        _, ds = utils.get_datasets(model_args, overwrite_data_root = args.data_root)
    ...
    with torch.no_grad():
        ...
        for i in range(len(all_codes)):
            print(f"Quantizing batch {i+1}/{len(all_codes)}")          
            ...
            if args.evaluation_dataset in ["KODAK"] or args.image_index is not None:
                #reconstruct the image for KODAK or if a single index is given
                title = f"Image {i} (2,4,8,16) = quantized, 32 = not quantized, 64 = original, 128 = residual"
            
                for k in range(len(args.num_bits)):
                    b = args.num_bits[k]
                    writer.add_image(title, make_grid(compressed_all[k], nrow = 24), global_step=b)
                writer.add_image(title, make_grid(no_quantized, nrow = 24), global_step=32)
                writer.add_image(title, make_grid(gt_img, nrow = 24), global_step=64)
                residual = torch.abs(gt_img-compressed_all[2]).sum(dim=1, keepdim = True)
                writer.add_image(title, make_grid(residual.repeat(1,3,1,1), nrow = 24), global_step=128)
                
            elif args.evaluation_dataset == "DIV2K":
                pass
                #to save disk space
            else:
                #store on tensorboard the reconstructed images
                title = "_".join(["Original"]+[str(b)+"bits" for b in args.num_bits]+["noQuantized", "residual"])
                to_grid = torch.cat((gt_img, *compressed_all, no_quantized, no_quantized-gt_img), dim=-1)
